...
//        HAL_GPIO_Init(ATIM_TIMX_PWM_CH3_GPIO_PORT, &gpio_init_struct);
        ...
        HAL_NVIC_SetPriority(ATIM_TIMX_INT_IRQn, 1, 1);
    ...
    if(htim->Instance==TIM1)
    {
        ...
        if(htim->Channel==HAL_TIM_ACTIVE_CHANNEL_1)
        {
            ...
            if(i1 % 2 == 0)
            {
                ...
                i1 = 0;
                g_run_flag1 = 1;                             /* 标志位置一 */
                g_stepper1.pulse_count--;                    /* 每一个完整的脉冲就-- */
                if(g_stepper1.dir == CW)
                {
                   g_stepper1.add_pulse_count++;             /* 绝对位置++ */
                }else
                {
                   g_stepper1.add_pulse_count--;             /* 绝对位置-- */
                }

                if(g_stepper1.pulse_count<=0)                /* 当脉冲数等于0的时候 代表需要发送的脉冲个数已完成，停止定时器输出 */
                {
            ...
            /*获取当前计数*/
            g_count_val1 =__HAL_TIM_GetCompare(&g_atimx_handle,TIM_CHANNEL_1);
            __HAL_TIM_SET_COMPARE(&g_atimx_handle, ATIM_TIMX_PWM_CH1, (g_count_val1 + 150)%0XFFFF);
        }
        if(htim->Channel==HAL_TIM_ACTIVE_CHANNEL_4)
        {
            ...
            if(i4 % 2 == 0)
            {
                i4 = 0;
                g_run_flag4 = 1;                             /* 标志位置一 */
                g_stepper4.pulse_count--;                    /* 每一个完整的脉冲就-- */
                if(g_stepper4.dir == CW)
                {
                   g_stepper4.add_pulse_count++;             /* 绝对位置++ */
                }else
                {
                   g_stepper4.add_pulse_count--;             /* 绝对位置-- */
                }

                if(g_stepper4.pulse_count<=0)                /* 当脉冲数等于0的时候 代表需要发送的脉冲个数已完成，停止定时器输出 */
                {
            ...
            /*获取当前计数*/
            g_count_val4 = __HAL_TIM_GET_COUNTER(&g_atimx_handle);
            __HAL_TIM_SET_COMPARE(&g_atimx_handle, ATIM_TIMX_PWM_CH4, (g_count_val4 + 150)%0XFFFF);
        ...
        if(htim->Channel==HAL_TIM_ACTIVE_CHANNEL_2)
        {
            ...
            if(i2 % 2 == 0)
            {
                ...
                i2 = 0;
                g_run_flag2 = 1;                             /* 标志位置一 */
                g_stepper2.pulse_count--;                    /* 每一个完整的脉冲就-- */
                if(g_stepper2.dir == CW)
                {
                   g_stepper2.add_pulse_count++;             /* 绝对位置++ */
                }else
                {
                   g_stepper2.add_pulse_count--;             /* 绝对位置-- */
                }

                if(g_stepper2.pulse_count<=0)                /* 当脉冲数等于0的时候 代表需要发送的脉冲个数已完成，停止定时器输出 */
                {
            ...
            /*获取当前计数*/
            g_count_val2 =__HAL_TIM_GetCompare(&g_atimx_handle,TIM_CHANNEL_2);
            __HAL_TIM_SET_COMPARE(&g_atimx_handle, ATIM_TIMX_PWM_CH2, (g_count_val2 + 150)%0XFFFF);
        }
    }    
}
