...
STEPPER_MOTOR g_stepper1 = {0};
STEPPER_MOTOR g_stepper2 = {0};
STEPPER_MOTOR g_stepper4 = {0};
...
void stepper_stop(uint8_t motor_num)
{
    switch(motor_num)
    {
        case STEPPER_MOTOR_1 :
        {
            HAL_TIM_OC_Stop_IT(&g_atimx_handle, ATIM_TIMX_PWM_CH1);     /* 关闭对应PWM通道 */
            break;
        }
        case STEPPER_MOTOR_2 :
        {
            HAL_TIM_OC_Stop_IT(&g_atimx_handle, ATIM_TIMX_PWM_CH2);     /* 关闭对应PWM通道 */
            break;
        }
        case STEPPER_MOTOR_4 :
        {
            HAL_TIM_OC_Stop_IT(&g_atimx_handle, ATIM_TIMX_PWM_CH4);     /* 关闭对应PWM通道 */
            break;
        }
        default : break;
    }
...
{
    g_stepper4.pulse_count = angle / MAX_STEP_ANGLE;
    if(g_stepper4.pulse_count == 0) 
    {
        stepper_stop(motor_num);
    }
    else stepper_star(motor_num,dir);
