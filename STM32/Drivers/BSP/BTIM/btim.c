...
{
    g_tim3_handle.Instance = BTIM_TIM3_INT;                      /* 通用定时器X */
    g_tim3_handle.Init.Prescaler = psc;                          /* 设置预分频系数 */
    ...
    HAL_TIM_Base_Start_IT(&g_tim3_handle);    /* 使能定时器x及其更新中断 */
}
...
void HAL_TIM_Base_MspInit(TIM_HandleTypeDef *htim)
{
    if (htim->Instance == BTIM_TIMX_INT)
    {
        ...
        HAL_NVIC_SetPriority(BTIM_TIMX_INT_IRQn, 3, 3); /* 抢占1，子优先级3，组2 */
        HAL_NVIC_EnableIRQ(BTIM_TIMX_INT_IRQn);         /* 开启ITM3中断 */
    }
    if (htim->Instance == BTIM_TIM3_INT)
    {
        BTIM_TIM3_INT_CLK_ENABLE();                     /* 使能TIM时钟 */
        HAL_NVIC_SetPriority(BTIM_TIM3_INT_IRQn, 2, 2); /* 抢占1，子优先级3，组2 */
...
{
    HAL_TIM_IRQHandler(&g_tim3_handle); /* 定时器中断公共处理函数 */
}
