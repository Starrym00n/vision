...
# 定义 GPIO 端口
GPIO_PORT = 18  # 例如 GPIO 17
# 设置 GPIO 方向为输入
GPIO.setup(GPIO_PORT, GPIO.IN)
serialPort = serial.Serial("/dev/ttyAMA2", 9600)
# 定义串口配置
or_point = [(0,0)]*9
sorted_points = [(0,0)]*9
picam2 = Picamera2()
    ...
    if GPIO.input(GPIO_PORT) == 0:
        ...
        if mode == 1:
            ...
            if(len(rectangles) > 5):
                ...
                for i in range(9):
                    ...
                    # print("矩形",i+1,"的中心点是", center_point)
                    # cv2.imshow("Original Image1", img)

                # try:
                #     uart = "矩形"+str(i+1)+"的中心点是"+ str(center_point)
                #     # 发送数据
                #     serialPort.write(uart.encode())
                # except Exception as e:
                #     print(e)


                # print("Rectangle:", rect_num) # 分别打印每个矩形的顶点坐标

                # for point in rect:
                #     print(point[0])

                # cv2.imshow("Original Image", img)# 显示处理后的图像
                mode = 2
            # 显示原图和处理后的图像
        ...
        if mode == 2:
            ...
            for i in range(9):
                ...
                # 打印结果
                # print(str(i+1)+"color:", most_common_color)
                # cv2.imshow("Original Image", img)
                # cv2.imshow("Region Around Point"+str(i+1), region)
        ...
        if(len(rectangles) == 1): # 如果检测到矩形数量等于0，则输出矩形顶点坐标
